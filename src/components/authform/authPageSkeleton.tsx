...
const AuthPageSkeleton: React.FC<PropsWithChildren<IProps>> = ({ children, title, callingUrl }) => {
  return (
    <div className='flex justify-center items-center min-h-[calc(100vh_-_4rem)]'>
      <div className='w-3/4 sm:w-1/2 md:w-2/5 xl:w-1/3 2xl:w-1/4 3xl:w-1/5 text-sm'>
        ...
        <div className='shadow-md dark:shadow-slate-600 rounded-md p-4 bg-gray-50 dark:bg-gray-800'>
          ...
          <nav className='pt-2'>
            {callingUrl === 'login' ? (
              <p className='mt-2 flex items-center' data-testid='login__link'>
                Login as&nbsp;
                <Link href='/login?role_type=staff' className='text-blue-500 underline'>
                  staff
                </Link>
              </p>
            ) : null}
            {callingUrl === 'staff-login' ? (
              <p className='mt-2 flex items-center' data-testid='staff-login__link'>
                Login as&nbsp;
            ...
            {callingUrl === 'login' || callingUrl === 'staff-login' || callingUrl === 'otp-login' ? (
              <p className='mt-2 flex items-center' data-testid='no-account-signup__link'>
            ...
            {callingUrl === 'otp-login' ? (
              <p className='mt-2 flex items-center' data-testid='otp-login__link'>
                Login as&nbsp;
                <Link href='/login?role_type=staff' className='text-blue-500 underline'>
                  staff
                </Link>
            ...
            {callingUrl === 'signup' ? (
              <p className='mt-2 flex items-center' data-testid='account-login__link'>
            ...
            {callingUrl === 'otp-login' ? null : (
              <>
                <p className='mt-2 flex items-center'>
                  <Link href='/login-using-otp' className='text-blue-500 underline'>
                    Forget password
                  </Link>
                </p>
                <p className='mt-2 flex items-center'>
